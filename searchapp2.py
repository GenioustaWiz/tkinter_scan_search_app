...
def search_files(query):
    ...
    for folder in cache['folders']:
        if query.lower() in folder.lower() and not any(exclude_folder in os.path.dirname(folder) for exclude_folder in exclude_folders):
            results.append(os.path.join('/', folder))
    return results
